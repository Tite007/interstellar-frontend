// src/app/(customer)/categories/[category]/[subcategory]/[productSlug]/MainProductDetailsClient.jsx
'use client'
...
export default function MainProductDetailsClient({ initialProduct }) {
  ...
  const [selectedPackage, setSelectedPackage] = useState(
    initialProduct?.size || null,
  )
  const [selectedDelivery, setSelectedDelivery] = useState('')
  const [price, setPrice] = useState(initialProduct?.price || null)
  ...
  const fetchProductDetails = async (productId) => {
    try {
      ...
      setIsOutOfStock(productData.currentStock === 0)
    } catch (error) {
  ...
  const handlePackageChange = (value) => {
    ...
    if (Array.isArray(product.variants) && product.variants.length > 0) {
      ...
      if (selectedVariant) {
        ...
        setIsOutOfStock(variantOption.quantity === 0)
        return
      }
    }
    ...
    setIsOutOfStock(product.currentStock === 0)
  }
